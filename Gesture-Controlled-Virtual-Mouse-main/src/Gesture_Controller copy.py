...
class Controller:
    ...
    def left_click(self):
        """
        Perform left-click when the left-click gesture is detected.
        """
    ...
    def scrollVertical():
        ...
        pyautogui.scroll(300 if Controller.pinchlv>0.0 else -300)
    ...
    def handle_controls(gesture, hand_result):
        ...
        # Get hand position if the gesture is not PALM
        if gesture != Gest.PALM:
            x, y = Controller.get_position(hand_result)

        # Release left-click if the gesture is not FIST and grabflag is set
        if gesture != Gest.FIST and Controller.grabflag:
            Controller.grabflag = False
            pyautogui.mouseUp(button="left")

        # Reset pinchmajorflag if the gesture is not PINCH_MAJOR
        if gesture != Gest.PINCH_MAJOR and Controller.pinchmajorflag:
            Controller.pinchmajorflag = False

        # Reset pinchminorflag if the gesture is not PINCH_MINOR
        if gesture != Gest.PINCH_MINOR and Controller.pinchminorflag:
            Controller.pinchminorflag = False
        
        if gesture != Gest.LAST4 and Controller.drag_flag:
            Controller.drag_flag = False
        ...
        # elif gesture == Gest.LAST4:  # Altered left-click gesture
        #     if not Controller.left_click_flag:
        #         Controller.left_click_flag = True
        #         pyautogui.mouseDown(button="left")
        #     pyautogui.moveTo(x, y, duration=0.1)
        if gesture == Gest.FIST:  # Start drag
            if not dragging:
                dragging = True
                pyautogui.mouseDown(button='left')
                x, y = Controller.get_position(hand_result)
        ...
        elif gesture == Gest.PALM:  # Release drag
            if dragging == True:
                dragging = False
                pyautogui.mouseUp(button='left')
